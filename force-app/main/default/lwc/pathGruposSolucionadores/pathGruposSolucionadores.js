...
export default class pathGruposSolucionadores extends LightningElement {
    ...
    renderedCallback() {
        this.gruposSolucionadores.forEach((buildedGroup) => {
            if (buildedGroup.status == "current") {
                const qtdGrupos = this.gruposSolucionadores.length
                const grupoAtual = this.gruposSolucionadores.map(function(e) { return e.index; }).indexOf(buildedGroup.index);
                const tamanhoTela = (screen.width - 132) * 1.8
                const posicaoGrupo = tamanhoTela * grupoAtual / qtdGrupos

                if (this.template.querySelector('[data-id="scrollDiv"]'))
                    this.template.querySelector('[data-id="scrollDiv"]').scrollLeft = posicaoGrupo < 200 ? 0 : posicaoGrupo;
            }
        })
    }
    ...
    get gruposSolucionadores() {
        ...
        if ((grupos.length > 3 && this.isNumberBetween(screen.width, 0, 600)) ||
            ...
            this.divColScrollableClass = 'slds-col divScrollableClass'
